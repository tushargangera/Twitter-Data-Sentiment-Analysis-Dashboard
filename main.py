...
'''
THIS_FOLDER = 'C:\\Users\\tgangera.I-FLEX\\Downloads\\Twitter_Sentiment_Analysis_Dashboard'
'''
...
'''
csv_file=os.path.join(THIS_FOLDER, 'tweet_sentiment.csv')
...
# plt.savefig('All Topics.png', dpi=300, bbox_inches='tight')
...
'''
# replies_avg=int(df['tweet_retweet_count'].mean())
replies_avg=0
...
'''
wordcloud_dir=os.path.join(THIS_FOLDER, 'All_Topics.png')
...
image header
'''
word_cloud_header= html.Div(html.H1('Word Cloud by word',className= 'word-cloud-header',id='word_cloud_header',
                                    style=dict(fontWeight='bold', color='black',
                                               )),
...
dff=df.groupby('topic')['tweetId'].count()
dff=dff.reset_index()
print(dff)
...
donut_header= html.Div(html.H1('Tweets Distribution',className= 'date-chart-header',id='donut_header',
                                    style=dict(fontWeight='bold', color='black')),
                            style=dict(textAlign="center", width='100%'))
...
'''
donut = go.Figure(data=go.Pie(labels=dff['topic'], values=dff['tweetId'],hole=.3,showlegend=False,sort=False))
